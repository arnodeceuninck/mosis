    ...
    def __init__(self, fname):
        ...
        for net in root.iter(nm('net')):
            ...
            for arc in net.iter(nm('arc')):
                ...
                self.arcs[id] = (
                    get_name(attr['source']), get_name(attr['target']), (attr['type'] == 'tapnInhibitor'), weight)
                ...
                if self.arcs[id][0] in self.transitions and self.arcs[id][1] in self.transitions:
                    raise ValueError(
                        "Cannot connect transition %s to transition %s!" % (self.arcs[id][0], self.arcs[id][1]))
                ctr += 1
    ...
    def fire(self, trans):
        ...
        for aid in self._sa.get(trans, []):
            ...
            if target not in self.places:
                print("WARNING: No such place %s reachable from %s. Please recreate both to make your model valid." % (
                    target, trans))
                continue
...
if __name__ == '__main__':
    ...
    def check_type(value):
        ...
        raise argparse.ArgumentTypeError(
            "Invalid type: %s. Expected prefix of 'reachability' or 'coverability'." % value.lower())


    parser = argparse.ArgumentParser(
        description="Generate the Reachability/Coverability graph of a Petri Net or find the P-Invariants.")
    parser.add_argument('input', help="The *.tapn file.")
    parser.add_argument('output', nargs='?', type=argparse.FileType('wb'), default=None,
                        help="The output file for GraphViz. When omitted, the console is used instead.")
    parser.add_argument('type', type=check_type,
                        help="The kind of graph to construct. Must be a prefix of 'reachability' or 'coverability'. Casing will be ignored.")
    parser.add_argument('-t', '--title', default=None,
                        help="The title to display in the plot. When omitted, the type of the graph will be shown.")
    parser.add_argument('-s', '--start', default="box",
                        help="The GraphViz shape to use for the start node. Defaults to 'box'.")
    parser.add_argument('-m', '--markings', action='store_false', help="Do not to print the marking place information.")
    parser.add_argument('-T', '--table', action='store_true',
                        help="Store the markings in a table.csv file, instead of using the node labels.")
    parser.add_argument('-I', '--iterations', type=check_positive, default=0,
                        help="The maximal amount of iterations to use. For Reachability, the default is 1000; "
                             "for Coverability, no limit is set by default.")
    parser.add_argument('-p', '--invariants', action='store_true',
                        help="Print out the P-Invariant formulas for the net.")
    parser.add_argument('-g', '--graphviz', action='store_true',
                        help="When used, the script outputs a (net.dot) graphviz file that represents the internal net. This can be used for checking inconsistencies between the TAPAAL visualization and how it is stored.")
